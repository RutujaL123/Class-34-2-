...
function keyPressed() {
  if (keyCode === LEFT_ARROW) {
   ...
   var ONEpackage = new packageBody(packageSprite.x, packageSprite.y);
...
function showPackages(packageSprite,i){
	if(packageSprite){
...
for (var i=0; i< packageSprite.length; i++) {
	showPackages(packages[i],i);
}

function keyReleased(){
	if(keyCode === DOWN_ARROW) {
		packages[packages.length-1].drop();
	}
}
